...
def weighted_sum_fwd(
	...
	for i in range(tl.ceil_div(D, D_TILE_SIZE)):
		...
		x_block = tl.load(x_block_ptr, boundary_check=(0, 1), padding_option="zero") # (ROWS_TILE_SIZE, D_TILE_SIZE)
		weight_block = tl.load(weight_block_ptr, boundary=(0,), padding_option="zero") # (D_TILE_SIZE, )

		output += x_block * tl.sum(x_block * weight_block[None, :], axis=1)
