...
from impl.weighted_sum_fwd import weighted_sum_fwd
from impl.weighted_sum_backward import weighted_sum_backward
...
class WeightedSumFunc(torch.autograd.Function):
	...
	def backward(ctx, grad_out):
		...
		# The number of rows in this buffer is equal to the number of program blocks.
		num_row_tiles = tl.cdiv(n_rows, ROWS_TILE_SIZE)
		partial_grad_weight = torch.empty((num_row_tiles, D), device=x.device, dtype=x.dtype)
		...
		# Final reduction across the partial results to get the (D,) weight gradient
